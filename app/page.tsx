  ...
  return (
    <main>
      ...
      <section className="experience" id="experiencia">
        <div className="experience-intro"><p className="eyebrow">04 · Experiência AJA</p><h2>Você não escolhe<br />um perfume em <em>dois segundos.</em></h2><p>A escolha que fica pede tempo na pele, contexto e repertório. Por isso, a Experiência AJA começa antes do frasco original.</p></div>
      ...
      <section className="quiz-section" id="quiz">
        <div className="quiz-copy"><p className="eyebrow">03 · Curadoria pessoal</p><h2>Qual presença<br />você quer <em>deixar?</em></h2><p>Conte o que procura. Nós cruzamos intenção, ocasião e intensidade para indicar uma primeira direção - e, se quiser, desenhar a continuação dessa conversa.</p></div>
      ...
      <section className="feedback" id="feedback">
        ...
        <form onSubmit={sendFeedback}>
          ...
          <div className="rating-field"><span className="field-label">Sua nota</span><div role="group" aria-label="Nota da fragrância de 1 a 10">{Array.from({ length: 10 }, (_, index) => index + 1).map((rating) => <button type="button" className={feedbackRating >= rating ? "active" : ""} aria-pressed={feedbackRating === rating} key={rating} onClick={() => { setFeedbackRating(rating); setFeedbackError(false); }}>{rating}</button>)}</div></div>
